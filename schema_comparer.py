...
class SchemaComparer:
    ...
    def compare_schemas(self):
        self.url_template.get_metadata()
        self.url_compare.get_metadata()

        self.url_template.check_required_keys()

        print(f"\nMetadata for {self.url_template.url} (Product type):")
        print(json.dumps(self.url_template.metadata, indent=2))

        print(f"\nMetadata for {self.url_compare.url} (Product type):")
        print(json.dumps(self.url_compare.metadata, indent=2))

        if self.url_template.metadata and self.url_compare.metadata:
            missing_keys_template = self.find_missing_keys(self.url_template.metadata, self.url_compare.metadata)
            missing_keys_compare = self.find_missing_keys(self.url_compare.metadata, self.url_template.metadata)

            print(f"\nMissing Keys in {self.url_template.url} compared to {self.url_compare.url}:")
            print(missing_keys_template)
            empty_keys_template = self.find_empty_values(self.url_template.metadata)
            if empty_keys_template:
                print("\nKeys with empty values:")
                print(empty_keys_template)

            print(f"\nMissing Keys in {self.url_compare.url} compared to {self.url_template.url}:")
